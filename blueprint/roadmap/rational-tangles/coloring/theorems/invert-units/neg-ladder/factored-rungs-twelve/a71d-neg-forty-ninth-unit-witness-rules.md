...
lean: RationalTangles.sharpColAddNegFortyNinth_witness_rules
proof: formalized
---

# Negative forty-nine-unit witness rule preliminaries

Arc equations for the forty-nine-unit witness coloring
`sharpColAddNegFortyNinth`: the conjunction of the forty-nine switched-crossing
...
under the span limit. Without this split the rung-forty-nine existence
projects past the 200 limit (the forty-nine one-line `exact` steps plus the
...
preliminaries (~114 lines). Carries `set_option maxRecDepth 4096`, matching the crossings
