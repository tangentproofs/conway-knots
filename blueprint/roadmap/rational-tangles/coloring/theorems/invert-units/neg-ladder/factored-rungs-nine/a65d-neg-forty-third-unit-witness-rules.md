...
lean: RationalTangles.sharpColAddNegFortyThird_witness_rules
proof: formalized
---

# Negative forty-three-unit witness rule preliminaries

Arc equations for the forty-three-unit witness coloring
`sharpColAddNegFortyThird`: the conjunction of the forty-three switched-crossing
...
under the span limit. Without this split the rung-forty-three existence
projects past the 200 limit (the forty-three one-line `exact` steps plus the
...
preliminaries (~102 lines). Carries `set_option maxRecDepth 4096`, matching the crossings
