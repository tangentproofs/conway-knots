...
lean: RationalTangles.sharpColAddNegFortieth_witness_rules
proof: formalized
---

# Negative forty-unit witness rule preliminaries

Arc equations for the forty-unit witness coloring
`sharpColAddNegFortieth`: the conjunction of the forty switched-crossing
...
under the span limit. Without this split the rung-forty existence
projects past the 200 limit (the forty one-line `exact` steps plus the
...
preliminaries (~99 lines). Carries `set_option maxRecDepth 4096`, matching the crossings
