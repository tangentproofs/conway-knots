...
Glue colorings of two independent `slideReady` diagrams (affine matching of
integral `colorFrom` along an `add` or `mul` seam, and the dual identity
`f(T*S)=1/(1/f(T)+1/f(S))`) are recorded for finite nonzero `F`. They are the
bookkeeping needed to compare `(T+S)ⁱ` with `Sⁱ * Tⁱ` when `T+S` is not a
`TwistExpr`. That assembled comparison is not yet a theorem: identifying the
restriction of a glue coloring with `colorAddRight` on the inverted
mirror-sum remains. This is not unrestricted `flype_slide`, and it is not
Theorem 2.
