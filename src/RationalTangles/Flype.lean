...
/-- A rational (local) flype preserves tangle isotopy: the easy direction of
    Proposition 4 (Kauffman–Lambropoulou). No alternating hypothesis is needed. -/
theorem isotopic_of_flype {D E : TangleDiagram} (h : IsLocalFlype D E) :
    Isotopic D E :=
  .flype h

/-- Finite sequences of rational flypes preserve isotopy, by transitivity. -/
theorem isotopic_of_flypeSeq {D E : TangleDiagram}
    (h : Relation.ReflTransGen IsLocalFlype D E) : Isotopic D E := by
  induction h with
  | refl => exact .refl _
  | tail _ h2 ih => exact ih.trans (.flype h2)
