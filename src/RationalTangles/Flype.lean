...
This is *not* `Isotopic`: it omits algebraic `Flype` (`hflip`/`vflip`+switch),
flip/mirror/invert congruence, associativity, transfer, and `ColoringIsotopy.symm`.
Indexed R3 is included via the local model; `add_zero` and `invert_unit` are
included. Invariance of `f` is stated for `ColoringIsotopy` rather than for
`Isotopic`.
-/
